...
export interface CultivationMethodEffectDefinition {
  id: string
  title: string
  body: string
  icon?: string
  tags?: string[]
  state?: 'core' | 'passive' | 'trigger' | 'support'
}
...
export interface CultivationMethodDefinition {
  ...
  description: string
  effects: CultivationMethodEffectDefinition[]
}
...
export const CULTIVATION_METHODS: CultivationMethodDefinition[] = [
  {
    ...
    effects: [
      {
        id: 'dragon_blood_core',
        title: '龙战于野',
        body: '每消耗一定斗气就会获得可叠加的【血怒】状态，增强攻击、防御与斗气恢复。',
        icon: resolveAssetUrl('cult-effect-dragon-blood-core.webp'),
        state: 'core',
      },
    ],
  },
  ...
  },
  {
    ...
    effects: [
      {
        id: 'tiger_stripe_core',
        title: '虎煞·绝影',
        body: '每次闪避成功后获得可叠加的【虎煞】状态，提升最终伤害，受到伤害时移除所有层数。',
        icon: resolveAssetUrl('cult-effect-tiger-stripe-core.webp'),
      },
    ],
  },
  {
