...
头盔（helmet）、铠甲（armor）、左手（盾 shieldL）、右手（武器 weaponR）、双手武器（weapon2H，与双持互斥）、戒指（ring1、ring2）

### 6.2 主属性与副属性
* 每件装备有一个**主属性**，戒指的主属性随机为生命或斗气上限，根据部位固定：
...
* 地图页默认展示作战与休憩的主节点：左侧以地图列表承载所有已解锁的区域，右侧视当前选中类型切换成城市或野外视图。
* 城市地图作为“功能集散地”，每个地点用百分比坐标置于底图之上，承载商店、公会、任务板等常驻模块，点击即展开业务区域或传送目的地。
...
* 所有城市地图默认可达，第一张野外地图默认解锁，其余野外需依赖相应 BOSS 首杀完成后动态解锁，开新区域会在列表中显示。
