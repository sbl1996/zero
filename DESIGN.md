...
* LV1，EXP 0；500 GOLD、生命药水×5、技能药水x3、必杀药水x1。
* 初始装备：铁剑、铁甲、铁盔（具体装备见*装备列表*）
...
- 默认绑定：槽1=生命药水，槽2=技能药水，槽3=必杀药水；战外可重绑。
...
* `tough`：普通怪默认为 1.0，当前所有 BOSS 设为 1.5（仍服从*伤害与减伤*中的韧性定义）。
...
**槽内类型权重（按槽逐个判定）：**
* 普通怪（按等级段见*等级段通用池*）：以**药水/装备/金币包**为主，不再产出宝石，剩余概率不掉落任何东西。
* **BOSS**：每个槽按权重独立抽：
  * 装备：**55%**（带“预强化”与“越阶概率”，见下）
  * 宝石：**25%**（仅 BOSS 且受等级段门控，见*等级段通用池*）
  * 金币包（×2/×3）：**20**
...
* 锁定状态：第一个城市地图和第一个野外地图默认解锁，其余野外地图需满足*地图列表*中的解锁条件。
...
- 地图：左侧地图列表，右侧城市/野外双布局（见*地图系统*）；翡冷翠地点可进商店或切换地图，野外地图点击怪物即开战。
